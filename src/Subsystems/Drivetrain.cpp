...
void Drivetrain::SetUsePID(bool usePid)
{
	if(usePid==this->usePid) return;
	this->usePid = usePid;
	if(!usePid){
		frontWheel->Disable();
		leftWheel->Disable();
...
void Drivetrain::DriveWithInputs(double x, double y, double turn)
{
	...
	if(!usePid){
		...
		frontWheel->SetSetpoint(MAX_RATE*speeds[0]);
		backWheel->SetSetpoint(MAX_RATE*speeds[1]);
		leftWheel->SetSetpoint(MAX_RATE*speeds[2]);
		rightWheel->SetSetpoint(MAX_RATE*speeds[3]);
	}
}
...
void Drivetrain::Stop()
{
	if(!usePid){
		...
		frontWheel->SetSetpoint(0);
		backWheel->SetSetpoint(0);
		leftWheel->SetSetpoint(0);
		rightWheel->SetSetpoint(0);
	}
}
